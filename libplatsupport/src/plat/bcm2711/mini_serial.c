...
{
    if ((NULL == dev) || (NULL == aux)) {
        return -EINVAL;
    }

    int ret = 0;

    if (0 != (ret = aux->enable(aux, BCM2711_AUX_UART))) {
        ZF_LOGE("Failed to enable UART in AUX subsystem! %i", ret);
        return ret;
    }
...
static inline int mini_uart_disable(ps_chardevice_t *dev, aux_sys_t* aux, bool disable_aux)
{
    ...
    if (disable_aux) {
        ...
        if (0 != (ret = aux->disable(aux, BCM2711_AUX_UART))) {
            ZF_LOGE("Failed to disable UART in AUX subsystem! %i", ret);
            return ret;
        }
    }
...
static void mini_uart_handle_irq(ps_chardevice_t *dev)
{
    ...
    /*
     * TODO: handle IRQs properly: how to handle TX interrupts?
     */

    const int source = mini_uart_get_irq_source(dev);
    ZF_LOGD("Interrupt source: %i", source);
                   ...
                   ps_chardevice_t *dev)
{
    ...
    dev->irqs       = defn->irqs;
    dev->ioops      = *ops;
    dev->flags      = SERIAL_AUTO_CR | SERIAL_RX_NONBLOCKING;

    int ret = 0;

    /* Initialize AUX subsystem handle */
    if (0 != (ret = bcm2711_aux_sys_init(ops, &aux))) {
    ...
    if (0 != (ret = mini_uart_disable(dev, &aux, false))) {
        ZF_LOGE("Failed to disable mini-UART! %i", ret);
        return ret;
    }
    if (0 != (ret = mini_uart_disable_interrupts(dev))) {
        ZF_LOGE("Failed to disable interrupts! %i", ret);
        return ret;
    }

    /* TODO: setup baud? */
    // regs->mu_baud = something?;
    ...
    /* Flush FIFO buffers */
    if (0 != (ret = mini_uart_flush_fifos(dev))) {
        ZF_LOGE("Failed to flush mini-UART FIFOs! %i", ret);
        return ret;
    }

    /* Enable RX interrupt */
    if (0 != (ret = mini_uart_enable_rx_irq(dev))) {
        ZF_LOGE("Failed to enable mini-UART RX IRQ! %i", ret);
        return ret;
    }

    /* Enable */
    if (0 != (ret = mini_uart_enable(dev, &aux))) {
